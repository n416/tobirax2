            ...
            <div style="display:flex; justify-content:space-between; align-items:flex-start; flex-wrap:wrap; gap:1rem;">
              ...
              <div style="display:flex; gap:0.5rem; flex-wrap:nowrap; justify-content:flex-end;">
                ${a.status === 'pending' ? html`
                  ${amRejectBtn('却下', { 'data-action': 'reject-request', 'data-id': String(a.id) })}
                  ${amApproveBtn('承認', { 'data-action': 'approve-request', 'data-id': String(a.id) })}
                ` : ''}
              </div>
            </div>
      ...
      ${Modal({
        ...
        children: html`
          ...
          <div style="display: flex; justify-content: flex-end; gap: 1rem;">
            <button type="button" data-action="close-reject-modal" style="background: transparent; color: #64748b; border: 1px solid #cbd5e1; border-radius: 8px; padding: 0.5rem 1rem; font-weight: 600; cursor: pointer;">キャンセル</button>
            <button type="button" data-action="execute-reject" style="background: #ef4444; color: white; border: none; border-radius: 8px; padding: 0.5rem 1rem; font-weight: 600; cursor: pointer;">却下する</button>
          </div>
        `
      })}
