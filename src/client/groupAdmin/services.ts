...
window.renderServices = function() {
    ...
    el.innerHTML = list.map(function(s: Service) {
        ...
        return '<tr>'
            ...
            +   '<button type="button" title="タグ管理" data-action="open-service-tags-modal" data-service-id="' + s.id + '" data-service-name="' + encodeURIComponent(s.name) + '" style="background:transparent;border:none;color:#94a3b8;cursor:pointer;padding:7px;border-radius:50%;width:34px;height:34px;"><span class="material-symbols-outlined" style="font-size:18px;">local_offer</span></button>'
            +   (s.status === 'rejected' ? '<button type="button" data-action="reapply-service" data-service-id="' + s.id + '" style="background:#fff;border:1px solid #fecaca;color:#dc2626;cursor:pointer;padding:0.4rem 0.75rem;border-radius:6px;font-size:0.8rem;font-weight:600;margin-right:0.5rem;transition:all 0.2s;"><span class="material-symbols-outlined" style="font-size:16px;vertical-align:middle;margin-right:2px;">refresh</span>再申請</button>' : '')
            +   (s.status !== 'active' ? '<button type="button" data-action="manage-service-apps" data-service-id="' + s.id + '" data-service-name="' + encodeURIComponent(s.name) + '" style="background:transparent;border:1px solid #cbd5e1;color:#4f46e5;cursor:pointer;padding:0.4rem 0.75rem;border-radius:6px;font-size:0.8rem;font-weight:600;margin-right:0.5rem;transition:all 0.2s;"><span class="material-symbols-outlined" style="font-size:16px;vertical-align:middle;margin-right:2px;">settings_applications</span>' + ((window.i18n || {}).svcManageApps || 'アプリを組み込む') + '</button>' : '')
            +   '<button type="button" data-action="manage-roles" data-service-id="' + s.id + '" data-service-name="' + encodeURIComponent(s.name) + '" style="background:transparent;border:1px solid #cbd5e1;color:#047857;cursor:pointer;padding:0.4rem 0.75rem;border-radius:6px;font-size:0.8rem;font-weight:600;margin-right:0.5rem;transition:all 0.2s;"><span class="material-symbols-outlined" style="font-size:16px;vertical-align:middle;margin-right:2px;">manage_accounts</span>役割(ロール)を管理</button>'
            +   (s.status !== 'active' ? '<button type="button" title="削除" data-action="remove-service" data-service-id="' + s.id + '" style="background:transparent;border:none;color:#94a3b8;cursor:pointer;padding:7px;border-radius:50%;width:34px;height:34px;transition:all 0.2s;display:inline-flex;align-items:center;justify-content:center;vertical-align:middle;"><span class="material-symbols-outlined" style="font-size:18px;">delete</span></button>' : '')
            + '</td>'
            + '</tr>';
    }).join('');
};
        ...
        if (roles.length === 0) {
            ...
            roleListEl.innerHTML = roles.map(function(r: ServiceRole) {
                return '<div style="display:flex; justify-content:space-between; align-items:center; padding:0.5rem; border-bottom:1px solid #f1f5f9;">'
                    ...
                    + '</div>'
                    + '<button type="button" data-action="remove-role" data-role-id="' + r.id + '" class="material-symbols-outlined" style="color:#ef4444; background:none; border:none; cursor:pointer; font-size:18px;">delete</button>'
                    + '</div>';
            }).join('');
        }
    }
        ...
        if (sTags.length === 0) {
            ...
            appliedTagsEl.innerHTML = sTags.map(function(t: ServiceTag) {
                ...
                return '<span style="display:inline-flex; align-items:center; gap:0.25rem; font-size:0.8rem; padding:4px 8px 4px 10px; border-radius:999px; background:'+bg+'; color:'+color+'; border:1px solid '+(t.status==='active'?'#a7f3d0':(t.status==='rejected'?'#fecaca':'#fed7aa'))+';">' + label
                    + '<button type="button" title="外す" data-action="remove-service-tag" data-service-id="' + serviceId + '" data-tag-id="' + t.tag_id + '" style="background:none;border:none;color:'+color+';cursor:pointer;padding:0 2px;line-height:1;font-size:1.1rem;opacity:0.7;">×</button>'
                    + '</span>';
            }).join('');
        }
    }
