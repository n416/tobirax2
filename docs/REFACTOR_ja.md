...
- ⏸️ **F 保留**（2026-06-20 判断・低優先）— `c: any` の解消。挙動不変の純粋な型衛生で、
  新規テスト価値はゼロ（当初ゴールの「テスト容易性」は D/E で達成済み）。得るのは開発時の
  型安全のみ。`c: any` はサーバ全体に蔓延しておりフルは波及が読めないため、今は着手しない。
  **必要になった時に最小スライスから**：再着手するなら、まず DB 関数群
  （`checkPermission` / `getEntitlements` / `authenticateClient` / `issueOidcTokens`）だけを
  `AppContext` 型に寄せ、波及をその4関数に閉じ込めて `tsc` 緑を保てる範囲で止める。
  フル（サーバ側 `c: any` 全廃）は非推奨（コスト対効果が最も悪い）。

現在テストはユニット 100 件＋統合 27 件・全緑、`tsc --noEmit` も clean。
リファクタ A〜E は完了、F は保留。テスト整備の一連はここで一区切りとする。
