...
> 💡 **補足：Tobira は独立した OIDC IdP として動作します**
> 当初はモックとして開発されましたが、現在では暗号化鍵のローテーションやD1ベースのレート制限、TOTPなどを備えた本番運用可能なIdPに成長しています。OIDC仕様の詳細については [OIDC_MOCK_ja.md](OIDC_MOCK_ja.md) を参照してください。
...
OIDC 署名鍵の暗号化キー（`OIDC_KEK`）や、セッショントークン用の署名鍵（`JWT_SECRET`）など、本番で必須となるシークレットを自動生成して登録します。
**※セキュリティのため、本番環境（ENVIRONMENT="dev"以外）で未設定の場合はエラーとなり起動しません（フェイルクローズ）。**

```bash
# 自動でセキュアなランダム値を生成し、Cloudflareにシークレットとして登録します
